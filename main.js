(() => {
  ...
  const returnPreviousPage = () => {
    const newTodolist = tabButtonList();
    if (currentPage > Math.ceil(newTodolist.length / TASKS_ON_PAGE)) {
      currentPage = Math.ceil(newTodolist.length / TASKS_ON_PAGE);
    }
  };
